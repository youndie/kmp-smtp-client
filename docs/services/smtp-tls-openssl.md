...
* **Один раз наблюдался разовый провал E2E `STARTTLS`** сразу после пересоздания контейнеров;
  два последующих прогона того же бинарника прошли. Причина не найдена. Если повторится —
  смотреть на готовность сервера, а не на TLS.
