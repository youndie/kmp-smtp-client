...
* **Mailpit принимает почти всё**, поэтому тот же E2E гоняется ещё и против настоящего Postfix
  (сервис `postfix` в `docker-compose.yml`, отдельный шаг CI). Разница видна сразу: Postfix
  отвергает `example.com` с `556 5.1.10 nullMX` — домен публикует null MX, то есть прямо заявляет,
  что почту не принимает, — а Mailpit такое берёт молча. Поэтому адрес получателя в E2E задаётся
  переменной `SMTP_E2E_RECIPIENT`.
* **Postfix пришлось настроить, чтобы он не искал получателя в `/etc/aliases.lmdb`**, которого нет
  в образе: без `local_recipient_maps = static:all` он отвечает `451 Temporary lookup failure`.
  Пустое значение туда не передать — образ пропускает пустые переменные окружения.
