...
> **Статус: M1 закрыта.** Ядро протокола разобрано и покрыто 68 тестами: ответы сервера
> (в том числе многострочные и потоком), расширенные коды состояния, команды, разбор `EHLO`,
> адреса конверта, прозрачность данных. Отправить письмо ещё нельзя: сессия делается на M2,
> транспорт на M3, TLS на M4.
